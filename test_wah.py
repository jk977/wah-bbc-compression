...
def run_length(s, ws):
    return wah.run_length(str_to_bs(s), ws)


def encode_run(s, ws):
    t, e = wah.encode_run(str_to_bs(s), ws)
    return (bs_to_str(t), bs_to_str(e))


def encode_literal(s, ws):
    t, e = wah.encode_literal(str_to_bs(s), ws)
    return (bs_to_str(t), bs_to_str(e))
...
class TestWAH(ut.TestCase):
    def test_run_length(self):
        for ws in range(8, 65):
            # test for no runs
            self.assertEqual(run_length(BitArray(), ws), 0)

            for s in '0', '1', '01', '10', '01'*1000:
                self.assertEqual(run_length(s, ws), 0)

        for ws in range(8, 65):
            # test for runs that don't overflow
            sect = ws - 1
            self.assertEqual(run_length('0'*sect, ws), 1)
            self.assertEqual(run_length('0'*sect*5, ws), 5)
            self.assertEqual(run_length('0'*sect*63, ws), 63)

        # test for overflows
        self.assertEqual(run_length('0000000'*64, 8), 63)
        self.assertEqual(run_length('000000000000000'*16384, 16), 16383)

    def test_encode_run(self):
        # test for single-word runs without leftovers
        self.assertEqual(encode_run('111', 4), ('', '1101'))
        self.assertEqual(encode_run('0000000', 8), ('', '10000001'))
        ...
        self.assertEqual(encode_run('0000000000', 8), ('000', '10000001'))
        self.assertEqual(encode_run('00000001111111', 8),
                         ('1111111', '10000001'))

        # test for multi-word runs without leftovers
        self.assertEqual(encode_run('111111', 4), ('', '1110'))
        self.assertEqual(encode_run('111111111', 4), ('', '1111'))

        self.assertEqual(encode_run('00000000000000', 8), ('', '10000010'))
        self.assertEqual(encode_run('1111111'*(2**6 - 1), 8), ('', '11111111'))
        ...
        self.assertEqual(encode_run('1111111110', 4), ('0', '1111'))
        self.assertEqual(encode_run('0000000'*2**6, 8),
                         ('0000000', '10111111'))
        self.assertEqual(encode_run('0000000'*(2**6 - 1) + '10101', 8),
                         ('10101', '10111111'))
    ...
    def test_encode_literal(self):
        for i in range(3, 65):
            ...
            self.assertEqual(encode_literal('1', i),
                             ('', '01' + '0'*(i - 2)))
            self.assertEqual(encode_literal('10', i),
                             ('', '01' + '0'*(i - 2)))
        ...
        self.assertEqual(encode_literal('110100110', 8),
                         ('10', '01101001'))
        self.assertEqual(encode_literal('1001110100', 8),
                         ('100', '01001110'))
